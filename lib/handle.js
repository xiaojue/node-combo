...
(function(ex) {
	if (ex) {
						...
						if (req.headers[ifModifiedSince] && lastModified == req.headers[ifModifiedSince]) {
							...
							if (req.headers.range) {
								...
								if (Isrange) {
									...
									compressHandle(raw, 206, "Partial Content", ext, req, rep);
								} else {
								...
								raw = fs.createReadStream(filepath);
								compressHandle(raw, 200, "Ok", ext, req, rep);
							}
						}
					}
				}
		...
		//compress
		function compressHandle(raw, statusCode, reasonPhrase, ext, req, rep) {
			var stream = raw,
